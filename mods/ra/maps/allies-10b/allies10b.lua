	...
	Trigger.OnEnteredFootprint(TanyaFootprint, function(actor, id)
		if actor.Owner == Greece and not tanyaTriggered then
			...
			Trigger.AfterDelay(DateTime.Seconds(6), function()
				...
				TanyaArrived = true
				TanyaSurvive = Greece.AddObjective("Tanya must survive.")
...
DeactivateMissiles = function()
	...
	Trigger.OnEnteredFootprint(TopLeftConsole, function(actor, id)
		if actor.Type == "e6" and not TopLeftTriggered then
			...
			Media.DisplayMessage("Nuclear missile deactivated.", "Console")
		end
	end)
...
Tick = function()
	...
	if ticked > 0 then
		UserInterface.SetMissionText("Nuclear missiles reach targets in " .. Utils.FormatTime(ticked), TimerColor)
		ticked = ticked - 1
...
WorldLoaded = function()
	...
	KillGreece = USSR.AddObjective("Stop the Allies.")
	StopNukes = Greece.AddObjective("Get your engineers to each launch control center.")
	Paperclip = Greece.AddObjective("Spare the scientists. They are more valuable as\nprisoners of war.", "Secondary", false)
